        ...
        for step in range(max_ep_len):
            ...
            with torch.no_grad():
                ...
                if args.ai:
                    ...
                    episode_data["observations"].append(next_obs.cpu().numpy()[0].tolist())
                    episode_data["actions"].append(action.cpu().numpy()[0].tolist())
                    ...
                    episode_data["observations"].append(p1_obs.cpu().numpy()[0].tolist())

                    p1_action, _, _, _, _ = agent.get_action_and_value(
                        p1_obs, envs=envs, invalid_action_masks=p1_mask, device=device
                    )

                    episode_data["actions"].append(p1_action.cpu().numpy()[0].tolist())
